...
export default function App() {
  ...
  const refresh = () => invoke<Bootstrap>("bootstrap").then((value) => {
    setBootstrap(value);
  ...
  useEffect(() => {
    ...
    const unlistenRuntime = listen<RuntimeStatus>("runtime-status", (event) => setRuntime(event.payload));
  ...
  useEffect(() => {
    ...
    const retryReadiness = () => {
      ...
      void Promise.all([
        shortcut,
        invoke<boolean>("enable_auto_insert", { prompt: false }),
      ]).then(([shortcutReady, autoInsertReady]) => {
        if (shortcutReady !== bootstrap.shortcutReady || autoInsertReady !== bootstrap.autoInsertReady) void refresh();
      });
    };
    window.addEventListener("focus", retryReadiness);
    return () => window.removeEventListener("focus", retryReadiness);
  }, [bootstrap?.shortcutReady, bootstrap?.autoInsertReady]);

  useEffect(() => {
    if (!bootstrap || bootstrap.platform !== "MACOS" || bootstrap.autoInsertReady) return;
    const modelReady = bootstrap.models.some((item) =>
      item.id === bootstrap.data.settings.modelId && item.downloaded,
    );
  ...
  }, [bootstrap?.platform, bootstrap?.autoInsertReady, bootstrap?.data.settings.modelId, bootstrap?.models]);

  if (!bootstrap) {
    return <div className="boot"><BrandMark /><span>Preparing Tertius</span></div>;
  }

  const model = bootstrap.models.find((item) => item.id === bootstrap.data.settings.modelId) ?? bootstrap.models[0];
  ...
  const shortcutName = bootstrap.platform === "MACOS"
    ? "Control + Option + V"
    : "Control + Alt + V";
  ...
  const enableShortcut = async () => {
    try {
      ...
      await refresh();
      if (!enabled) {
        setNotice("That shortcut is already reserved by another app. The dictation button is still available.");
      }
    } catch (error) {
      setNotice(friendlyError(error));
    }
    ...
    <div className={`simple-app platform-${bootstrap.platform.toLowerCase()}`}>
      ...
      <header className="app-header">
        <div className="identity">
          ...
          <div><strong>TERTIUS</strong><small>BY FARYNTH</small></div>
        </div>
        <nav className="app-tabs" aria-label="Tertius views">
          <button className={view === "dictate" ? "selected" : ""} onClick={() => setView("dictate")}>Dictate</button>
          <button className={view === "recent" ? "selected" : ""} onClick={() => setView("recent")}>Recent <span>{bootstrap.data.history.length}</span></button>
        </nav>
      </header>

      {view === "dictate" ? <main className="dictate-view">
        <section className={`dictation ${active ? "active" : ""}`}>
          <div className="dictation-copy">
            <small>{state.kicker}</small>
            <h1>{state.title}</h1>
            <p>{state.detail}</p>
            {!active && !working && <div className="hero-keycaps">{keys.map((key) => <kbd key={key}>{key}</kbd>)}</div>}
          </div>

          <button className={`dictation-button phase-${runtime?.phase ?? "idle"}`} onClick={() => void record()} disabled={Boolean(working)}>
            <TopographicBands phase={runtime?.phase ?? "idle"} level={runtime?.level ?? 0} />
            <span className="button-ring" />
            {download > 0 ? <DownloadProgress value={download} /> : working ? <WorkingMark /> : <BrandMark live={active} />}
            <strong>{download > 0 ? "Setting up" : state.button}</strong>
            <small>{download > 0 ? `${Math.round(download * 100)}%` : active ? formatClock(runtime?.elapsedMs ?? 0) : ready ? keys.join(" + ") : "ONE-TIME DOWNLOAD"}</small>
          </button>
        </section>

        <section className="shortcut-card">
          <div className="shortcut-copy">
            <small>{bootstrap.shortcutReady ? "SHORTCUT BEHAVIOR" : "SHORTCUT UNAVAILABLE"}</small>
            <p>{bootstrap.shortcutReady ? (activationMode === "hold" ? `Hold ${shortcutName} while you speak. Release when you are done.` : `Press ${shortcutName} to start. Press it again to finish.`) : "Another app may already own this shortcut. You can retry or use the dictation button."}</p>
          </div>
          <div className="shortcut-actions">
            {bootstrap.shortcutReady ? <div className="mode-picker" aria-label="Shortcut behavior">
              <button className={activationMode === "hold" ? "selected" : ""} onClick={() => void setActivationMode("hold")}>Hold</button>
              <button className={activationMode === "toggle" ? "selected" : ""} onClick={() => void setActivationMode("toggle")}>Press on / off</button>
            </div> : <button className="enable-shortcut" onClick={() => void enableShortcut()}>Retry shortcut</button>}
          </div>
        </section>
        ...
        {recent && (
          <section className="recent-output">
            ...
            <p>{recent.text}</p>
            <span>{recent.appName ?? "This device"} · {recent.words} words</span>
          </section>
        )}
      ...
      </main> : <main className="history-view">
        <section className="history-intro">
          <small>THREE-DAY LOCAL HISTORY</small>
          <h1>Recent dictations.</h1>
          <p>Finished text stays on this device for three days, then Tertius removes it automatically.</p>
        </section>
        {bootstrap.data.history.length > 0 ? <div className="history-list">
        ...
        </div> : <section className="history-empty">
          <BrandMark />
...
function statusCopy(runtime: RuntimeStatus | undefined, ready: boolean, mode: ActivationMode, shortcutReady: boolean) {
  if (!ready) return {
    kicker: "FIRST RUN",
    title: "Set up once.",
  ...
  if (runtime?.phase === "recording") return {
    kicker: runtime.mode === "pushToTalk" ? "LISTENING / RELEASE TO FINISH" : "LISTENING / PRESS AGAIN TO FINISH",
    title: "I’m listening.",
    detail: "Speak naturally. Tertius will clean the words and write them where your cursor is.",
    button: "Finish dictation",
  };
  if (runtime?.phase === "starting") return {
  ...
  if (runtime && !["idle", "error"].includes(runtime.phase)) return {
  ...
  return {
    ...
    title: "All set!",
    detail: mode === "hold" ? "Give it a try. Hold the keys below, speak, then release." : "Give it a try. Press the keys below to start and stop.",
    button: "Start dictation",
  };
}

function DownloadProgress({ value }: { value: number }) {
  return <span className="download-mark"><Download size={25} /><i style={{ transform: `scaleX(${value})` }} /></span>;
}

function WorkingMark() {
  return <span className="working-mark"><Mic2 size={30} /></span>;
}
...
function TopographicBands({ phase, level }: { phase: RuntimeStatus["phase"]; level: number }) {
  ...
  return (
    <span className={`topographic-bands ${listening ? "listening" : ""} ${composing ? "composing" : ""}`} aria-hidden="true">
...
function friendlyError(error: unknown) {
  ...
  if (message.toLowerCase().includes("timed out")) return "The microphone did not respond. Check Sound settings, then try again.";
  if (message.toLowerCase().includes("microphone") || message.toLowerCase().includes("input device")) return "Tertius needs microphone access to listen.";
  if (message.toLowerCase().includes("speech engine") || message.toLowerCase().includes("download")) return "Set up the local speech engine once, then try again.";
  if (message.toLowerCase().includes("shortcut")) return "That shortcut could not be registered. Another app may already be using it.";
