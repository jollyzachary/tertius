...
export default function Overlay() {
  ...
  useEffect(() => {
    ...
    const unlistenStatus = listen<RuntimeStatus>("runtime-status", (event) => setStatus(event.payload));
    ...
    const detectPointer = async () => {
      ...
      try {
        ...
        const inside = Math.abs(pointer.x - center.x) <= logical.width * scale / 2 + margin
  ...
  const initializePosition = async () => {
    ...
    const monitor = monitors.find((item) => item.name === stored?.monitorName)
  ...
  const resizeAroundCenter = async (open: boolean) => {
    const center = centerRef.current;
    if (!center) return;
    const monitors = await availableMonitors();
  ...
  const setWindowGeometry = async (center: { x: number; y: number }, open: boolean, monitor: Monitor) => {
    ...
    >
      <div className="voice-pill">
        ...
        <div className="voice-status">
          <span>{label}</span>
          <small>{recording ? formatClock(status.elapsedMs) : status.phase === "idle" ? "CLICK" : "LOCAL"}</small>
        </div>
        ...
        <div className="voice-tail">
          ...
          <div className="voice-controls">
            <button className="voice-grip" aria-label="Move dictation widget" title="Move" onPointerDown={(event) => void beginDrag(event)}>
              <GripVertical size={12} />
            </button>
            <button className="voice-close" aria-label="Close dictation widget" title="Close widget" onClick={() => void closeWidget()}>
              <X size={11} />
            </button>
          </div>
        </div>
...
function ClassicMicrophone() {
  return (
    <svg className="classic-mic" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <path d="M9.35 6h5.3M8.85 9h6.3M9.35 12h5.3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
      <path d="M5.5 10.4v.8a6.5 6.5 0 0 0 13 0v-.8M12 17.7v3.1M8.8 21h6.4" stroke="currentColor" strokeWidth="1.55" strokeLinecap="round" />
    </svg>
  );
}
...
function readPosition(): WidgetLocation | null {
  try {
    const stored = JSON.parse(localStorage.getItem(POSITION_KEY) ?? "null") as WidgetLocation | null;
...
function clampCenter(center: { x: number; y: number }, monitor: Monitor) {
  ...
  return {
    ...
    y: clamp(center.y, area.position.y + halfHeight, area.position.y + area.size.height - halfHeight),
  };
}
